...
- **登录后先记下所有能访问的敏感端点，退出登录后再逐一测试这些端点是否能未授权访问**
...
- **发现登录页 → 先用默认密码尝试**，10 秒内能确认是否有效
- **发现用户名枚举 → 立即密码喷洒**：1 个弱密码（如 123456）对 N 个用户名尝试，成功率远高于单用户爆破
- 喷洒最多试 5 组密码（123456 / admin123 / password / admin888 / 12345678），不必跑完整字典
...
- **发现 JS 文件先 analyze_js 再肉眼复核**——工具提取路由/路径/密钥，但它可能漏。工具跑完后挑关键文件人工审一遍，确认没有遗漏
...
- **SSRF 回连地址**: `http://43.136.92.170:9999/<random-path>` — 测试 SSRF 时让目标请求此地址，在服务器上 `tail /tmp/ssrf_listener.log` 查看是否收到请求
